...
// para escrever os valores, abrir editor de texto, clicar no botão acoplado ao arduino. Pode ser então salvao como .csv, por exemplo
...
int n_leituras = 100;    // número de leituras a cada vez que o r é pressionado - mudar isso para uma chave física? o número de sensores pode ser definido com uma chave de 3 posições?
int led = 10;
...
void loop()
{
  ...
  if(Serial.available()) {
   ...
   // quando pressionamos o botão, o arduino lê os dados da EEPROM e os escreve como se fosse um teclado em qualquer lugar que aceite texto
